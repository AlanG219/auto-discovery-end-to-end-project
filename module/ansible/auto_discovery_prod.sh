...
aws_discovery() {
    ...
    /usr/local/bin/aws ec2 describe-instances \
        --filters "Name=tag:aws:autoscaling:groupName,Values=pet-auto_prod_asg" \
        --query 'Reservations[*].Instances[*].NetworkInterfaces[*].PrivateIpAddress' \
        --output text > "/etc/ansible/prod-ips.list"
}
...
update_inventory() {
    ...
    while IFS= read -r instance; do
        ...
        echo "$instance ansible_user=ec2-user ansible_ssh_private_key_file=/home/ec2-user/.ssh/id_rsa" >> "/etc/ansible/prod-hosts"
    done < "/etc/ansible/prod-ips.list"
